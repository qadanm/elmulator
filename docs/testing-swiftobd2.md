...
SwiftOBD2's `BLEManager` uses the system `CoreBluetooth` types directly, so it can't be transparently driven by CoreBluetooth-Mock (and therefore by [`ElmulatorCoreBluetoothMock`](../docs/swift-package.md)) without a small change to SwiftOBD2 itself. Two options, smallest first:
